...
@triton.jit(
    do_not_specialize=["bs", "max_seq_pages", "num_block_table_pages", "page_size", "seq_len_offset", "pool_len"]
)
def _init_replay_block_tables_kernel(
    req_to_token_ptr,
    ...
    row = pid.to(tl.int64)
    req_idx = tl.load(req_pool_indices_ptr + row).to(tl.int64)
    seq_len = tl.load(seq_lens_ptr + row) + seq_len_offset
    tl.store(out_seq_lens_ptr + row, seq_len)

    req_row_ptr = req_to_token_ptr + req_idx * pool_len
    out_row_ptr = out_block_tables_ptr + row * out_block_table_stride
    page_offsets = tl.arange(0, BLOCK_PAGES)
    page_offsets_i64 = page_offsets.to(tl.int64)

    num_loops = tl.cdiv(num_block_table_pages, BLOCK_PAGES)
    for i in range(num_loops):
        pages = i * BLOCK_PAGES + page_offsets
        page_mask = pages < num_block_table_pages
        valid_mask = pages < max_seq_pages

        token_indices = tl.load(
            req_row_ptr + page_offsets_i64 * page_size + i * BLOCK_PAGES * page_size,
            mask=valid_mask,
            other=0,
        )
        block_indices = token_indices // page_size
        block_indices = tl.where(valid_mask, block_indices, 0)
        tl.store(out_row_ptr + pages.to(tl.int64), block_indices, mask=page_mask)


def init_replay_block_tables_npu_triton(
    ...
    _init_replay_block_tables_kernel[(bs,)](
        req_to_token,
        req_pool_indices,
        seq_lens,
        block_tables,
