...
class WelmV4FusedRMSNorm(MultiPlatformOp):
    def __init__(
    ...
    def forward_native(
        ...
        if residual is not None:
            hidden = hidden + residual.float()

        variance = hidden.square().mean(dim=-1, keepdim=True)
        normalized_fp32 = hidden * torch.rsqrt(variance + self.eps)
        normalized_fp32 = normalized_fp32 * self.weight.float()
        ...
        if clone_fp32_out:
            return output, out_residual, normalized_fp32
...
class WelmV4InplaceRotaryEmbedding(RotaryEmbedding):
    ...
    def forward_native(
        ...
        The base SGLang implementation rotates the first ``rotary_dim`` values
        of each head. WeLMv4 stores the rotary subspace at the end of each
        head, and KV-mirror prefill uses a shorter Q tensor whose positions are
        selected by ``last_index``.
        """
        assert fused_set_kv_buffer_arg is None, (
            "fused_set_kv_buffer_arg is not supported by WeLMv4 native RoPE"
        )
        if offsets is not None:
            positions = positions + offsets
        positions = positions.flatten()

        def apply_tail_rope(tensor: torch.Tensor, rope_positions: torch.Tensor):
            original_shape = tensor.shape
            tensor = tensor.view(-1, original_shape[-1] // self.head_size, self.head_size)
            passthrough = tensor[..., : self.head_size - self.rotary_dim]
            rotary = tensor[..., self.head_size - self.rotary_dim :]
            cos_sin = self.cos_sin_cache.index_select(0, rope_positions)
            cos, sin = cos_sin.chunk(2, dim=-1)
            rotary = self._apply_rotary_emb_wrapped(
                rotary, cos, sin, self.is_neox_style
            )
            return torch.cat((passthrough, rotary), dim=-1).reshape(original_shape)

        key = apply_tail_rope(key, positions)
        query_positions = positions if last_index is None else positions[last_index]
        query = apply_tail_rope(query, query_positions)
        return query, key
