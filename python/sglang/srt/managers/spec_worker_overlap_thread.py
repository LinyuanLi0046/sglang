...
class SpecModelWorkerOverlapClient:
    ...
    def _to_python_value(value):
        ...
        if isinstance(value, tuple):
            return tuple(SpecModelWorkerOverlapClient._to_python_value(v) for v in value)
        if isinstance(value, list):
            return [SpecModelWorkerOverlapClient._to_python_value(v) for v in value]
        return value

    def _materialize_scheduler_ready_result(
        self, batch_result: GenerationBatchResult
    ) -> GenerationBatchResult:
        if batch_result.next_token_ids is not None:
            batch_result.next_token_ids = self._to_python_value(batch_result.next_token_ids)
        if batch_result.accept_lens is not None:
            batch_result.accept_lens = self._to_python_value(batch_result.accept_lens)

        logits_output = batch_result.logits_output
        if logits_output is not None:
            if logits_output.next_token_logprobs is not None:
                logits_output.next_token_logprobs = self._to_python_value(
                    logits_output.next_token_logprobs
                )
            if logits_output.input_token_logprobs is not None:
                logits_output.input_token_logprobs = tuple(
                    self._to_python_value(logits_output.input_token_logprobs)
                )
            if logits_output.next_token_top_logprobs_val is not None:
                logits_output.next_token_top_logprobs_val = self._to_python_value(
                    logits_output.next_token_top_logprobs_val
                )
            if logits_output.next_token_top_logprobs_idx is not None:
                logits_output.next_token_top_logprobs_idx = self._to_python_value(
                    logits_output.next_token_top_logprobs_idx
                )
            if logits_output.next_token_token_ids_logprobs_val is not None:
                logits_output.next_token_token_ids_logprobs_val = self._to_python_value(
                    logits_output.next_token_token_ids_logprobs_val
                )
            if logits_output.next_token_token_ids_logprobs_idx is not None:
                logits_output.next_token_token_ids_logprobs_idx = self._to_python_value(
                    logits_output.next_token_token_ids_logprobs_idx
                )

        batch_result.copy_done = None
        batch_result.launch_done = None
        return batch_result
    ...
    def resolve_last_batch_result(
        ...
        batch_result = self._materialize_scheduler_ready_result(batch_result)
